...
| GET | `/payout/preview` | Накопленный кэшбэк по картам за месяц |
| POST | `/payout` | Начислить накопленный кэшбэк доходной операцией |
...
#### GET `/payout/preview` — query

| Параметр | Тип | Описание |
|----------|-----|----------|
| `period_month` | string | Месяц `YYYY-MM`; по умолчанию — прошлый месяц |

Ответ: `period_month` и `cards[]` с полями `card_id`, `card_name`, `account_id`, `accrued_amount` (накоплено за период) и `already_paid_out`.

#### POST `/payout`

Создаёт доходную операцию в категории **«Кэшбэк»** (создаётся автоматически, если её нет) по счёту карты, датированную сегодняшним днём.

| Поле | Тип | Обязательно | Описание |
|------|-----|-------------|----------|
| `card_id` | UUID string | да | Карта, по которой начислен кэшбэк |
| `period_month` | string | нет | Месяц `YYYY-MM`; по умолчанию — прошлый месяц |
| `amount` | decimal | нет | Сумма (> 0); по умолчанию — накопленный кэшбэк за период |

Повторное начисление за тот же месяц по той же карте возвращает `409`; после удаления созданной операции начисление можно провести заново. Если накопленного кэшбэка нет и `amount` не передан — `422`.
...
| GET | `/merchants` | Траты по мерчантам внутри категорий (карта магазинов) |
| GET | `/ratios` | Финансовые коэффициенты |

#### GET `/statistics`, `/heatmap` и `/merchants` — query
...
#### GET `/merchants` — что попадает в выборку

Только расходы с заполненным `merchant_name`. Категория **«Переводы людям»** исключается всегда — карта строится по магазинам, а переводы людям магазинами не являются. Категория «Инвестиции» исключается при `exclude_investments=true`, как и в остальной аналитике. Сопоставление — по точному имени категории пользователя.
